...
//发送http请求
static gboolean curl_service_get(const char* url, char* req_body, char (*header_list)[128],
                                 int header_cnt) {
    char header[1024];
    int i;
...
gboolean setup_gat1400(char* deviceID, char* serverip, char* user, char* password, char* up_user,
                       char* up_pwd) {
...
gboolean plc_register() {
    ...
    if (gat_http_client_open() == 0) {
        ...
        logger_info("The info name is %s_info, detail info is plc_register code: %ld", app_name,
                    resp_code);
        ...
        ret = FALSE;
        logger_warning("The exception name is %s_exception, detail info is open http-client fail!",
                       app_name);
    }
...
gboolean plc_faceUpload(FaceInfo* face) {
    ...
    json_object_object_add(imageInfo_json, "StoragePath",
                           json_object_new_string(face->imageInfo.StoragePath));
    ...
    json_object_object_add(imageInfo_json, "ShotTime",
                           json_object_new_string(face->imageInfo.ShotTime));
    ...
    if (face->imageInfo.imageData != NULL && face->imageInfo.imageSize != 0)
        imageData = g_base64_encode(face->imageInfo.imageData, face->imageInfo.imageSize);
    ...
    json_object_object_add(faceImageInfo_json, "StoragePath",
                           json_object_new_string(face->faceImageInfo.StoragePath));
    ...
    json_object_object_add(faceImageInfo_json, "ShotTime",
                           json_object_new_string(face->faceImageInfo.ShotTime));
    json_object_object_add(faceImageInfo_json, "Width",
                           json_object_new_int(face->faceImageInfo.Width));
    json_object_object_add(faceImageInfo_json, "Height",
                           json_object_new_int(face->faceImageInfo.Height));
    // std::string faceImageData = "";
    /*
    gchar *faceImageData = NULL;
    if (face->faceImageInfo.imageData != NULL && face->faceImageInfo.imageSize != 0) faceImageData =
    g_base64_encode(face->faceImageInfo.imageData, face->faceImageInfo.imageSize);

    // g_message("%s", faceImageData);
    logger_info("The info name is %s_info, detail info is imageSize: %d", app_name,
    face->faceImageInfo.imageSize); if (faceImageData != NULL) {
    ...
    */
    ...
    json_object_object_add(faceInfo_json, "LocationMarkTime",
                           json_object_new_string(face->LocationMarkTime));
    json_object_object_add(faceInfo_json, "FaceAppearTime",
                           json_object_new_string(face->FaceAppearTime));
    json_object_object_add(faceInfo_json, "FaceDisAppearTime",
                           json_object_new_string(face->FaceDisAppearTime));
    json_object_object_add(faceInfo_json, "AgeUpLimit", json_object_new_int(80));
    json_object_object_add(faceInfo_json, "AgeLowerLimit",
                           json_object_new_double(face->score)); // int(10));
    json_object_object_add(faceInfo_json, "AccompanyNumber",
                           json_object_new_int(face->AccompanyNumber));
    ...
    json_object_object_add(faceInfo_json, "Similaritydegree",
                           json_object_new_double(face->distance)); // 0));
...
gboolean plc_vehicleUpload(VehicleInfo* vehicle) {
    ...
    json_object_object_add(jMotorVehicleElem, "PlateColor",
                           json_object_new_string(vehicle->PlateColor));
    json_object_object_add(jMotorVehicleElem, "PlateReliability",
                           json_object_new_string(vehicle->PlateReliability));
    ...
    json_object_object_add(jMotorVehicleElem, "StorageUrl1",
                           json_object_new_string(vehicle->img_full.StoragePath));
    json_object_object_add(jMotorVehicleElem, "StorageUrl2",
                           json_object_new_string(vehicle->img_plate.StoragePath));
    ...
    if (vehicle->img_full.imageSize > 0) {
        ...
        // gchar *b64data = g_base64_encode(vehicle->img_full.imageData,
        // vehicle->img_full.imageSize);
        json_object_object_add(jSubImg, "ImageID", json_object_new_string(imgId));
        json_object_object_add(jSubImg, "StoragePath",
                               json_object_new_string(vehicle->img_full.StoragePath));
    ...
    if (vehicle->img_plate.imageSize > 0) {
        ...
        struct json_object* jSubImg = json_object_new_object();
        // gchar *b64data = g_base64_encode(vehicle->img_plate.imageData,
        // vehicle->img_plate.imageSize);
        json_object_object_add(jSubImg, "ImageID", json_object_new_string(imgId));
        json_object_object_add(jSubImg, "StoragePath",
                               json_object_new_string(vehicle->img_plate.StoragePath));
        json_object_object_add(jSubImg, "FileFormat", json_object_new_string("Jpeg"));
    ...
    if (gat_http_client_open() == 0) {
        ...
        if (resp_code == 200) {
            ...
            logger_warning("The warning name is %s_warn, detail info is invalid respond code:%ld",
                           app_name, resp_code);
        }
        gat_http_client_close();
    } else {
        logger_error("The error name is %s_error, detail info is fail to open http client",
                     app_name);
    }
...
static gboolean hk_get_img_server(const char* url) {
    ...
    if (gat_http_client_open() == 0) {
        ...
        if (resp_code != 200) {
            ...
            logger_info(">>>up-ip:%s,up-port:%d,url:%s", _hk_img_svr.upload_ip,
                        _hk_img_svr.upload_port, _hk_img_svr.requrl);
            logger_info(">>>down-ip:%s,down-port:%d,PoolID:%s", _hk_img_svr.download_ip,
                        _hk_img_svr.download_port, _hk_img_svr.pool);
        }
...
// static void hk_get_auth_code(const char *source_data, const char *digest_key, char *digest, int
// digest_size) {
...
static size_t hk_cb_data_writer(void* data, size_t size, size_t nmemb, void* userdata) {
    ...
    memcpy(g_img_upload_resp_buf, (char*) data,
           (totalSize > (GAT_RESP_SIZE - 1) ? (GAT_RESP_SIZE - 1) : totalSize));
...
static int curl_img_upload(const char* infodata, const char* bufdata, unsigned int bufsize,
                           char* img_url, unsigned int img_url_len) {
    CURL* curl;
    long respcode = 0;
...
int hk_img_upload(const char* bufdata, unsigned int bufsize, char* download_url,
